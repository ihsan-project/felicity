...
class StringExample extends Any {
    ...
    _generate(rules) {
        ...
        const specials = {
            ...
            pattern: () => {
                const pattern = rules.pattern.invert ? /[a-f]{3}/ : rules.pattern.regex;
    ...
    _generate(rules) {
        ...
        if (schemaDescription.flags) {
            ...
            if (schemaDescription.flags.format) {
                ...
                const targetFormat = Array.isArray(schemaDescription.flags.format) ?
                    Helpers.pickRandomFromArray(schemaDescription.flags.format) :
                    schemaDescription.flags.format;
...
class ObjectExample extends Any {
    ...
    _generate(rules) {
        ...
        if (rules.schema && rules.schema.type) {
    ...
    _generate(rules) {
        ...
        if (schemaDescription.alternatives.length > 1) {
            ...
            if (schemaDescription.alternatives[0].ref) {
                ...
                Helpers.descriptionCompiler(schemaDescription.alternatives[0].is).validate(driverValue, (err) => {
